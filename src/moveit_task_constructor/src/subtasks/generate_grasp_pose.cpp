...
#include <moveit_msgs/msg/display_robot_state.hpp>//#include <moveit_msgs/DisplayRobotState.h>
...
moveit::task_constructor::subtasks::GenerateGraspPose::compute(){
	...
	geometry_msgs::msg::Pose object_pose, grasp_pose;
	...
	while(current_angle_ < 2*M_PI){
		...
		bool succeeded= grasp_state.setFromIK(jmg, grasp_pose, eef_, remaining_time_, is_valid);
